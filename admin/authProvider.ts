...
const getToken = async (userName: string, password: string) => {
    ...
    if (response?.data?.result?.data) {
        localStorage.setItem('userData', JSON.stringify(response?.data?.result?.data));
    }
};
...
const authProvider: AuthProvider = {
    ...
    getPermissions: () => {
        if (localStorage.getItem('userData')) {
            ...
            const reg = userData?.user?.user?.registrations?.find((u: any) => u.applicationId === AuthApplicationID);
            if (reg?.roles?.length) {
