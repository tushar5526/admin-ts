...
const FilterWithMenuOption = (permissions: string[], menuOption: any) => {
    ...
    menuOption?.permissions?.forEach((permission: any) => {
        permissions.forEach((p: string) => {
            if (permission?.toLowerCase() === p.toLowerCase() && ResourcePermissions.canList) {
                found = true;
            }
        })
    })
    ...
    return found && menuOption.props ? {...menuOption, resourcePermissions: ResourcePermissions} : false;
}
export const ItemWithPermissionResolver = (permissions: any, resource: string) => {
    // Permissions are case In Sensitive
    if (permissions?.length && resource) {
        const menuOption = MenuOptions.find((menuOption) => menuOption.resource.toLowerCase() === resource.toLowerCase());
        return FilterWithMenuOption(permissions, menuOption)
    }
    return null;
}
